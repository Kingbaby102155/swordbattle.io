      ...
      ping("us1").then(res1 => {
        ...
        ping("eu1").then(res3 => {
          ...
          if (pings.filter(p => p.error).length == pings.length) {
            ...
            var scores = pings.map(p => (p.ping*2) - (p.info.actualPlayercount ? p.info.actualPlayercount * 50 : 0) + (p.info.lag == "No lag" ? 0 : p.info.lag == "Moderate lag" ? 250 : 1000) + (p.info.playerCount > 15 ? Math.abs(15-p.info.playerCount)*200 : 0)).map((p) => !p ? Infinity : p);
